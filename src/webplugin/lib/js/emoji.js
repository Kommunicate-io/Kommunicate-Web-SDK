...
(function () {
    ...
    // settings
    emoji.img_path = 'https://www.applozic.com/resources/emojify/images/';
    emoji.sheet_path = 'https://www.applozic.com/resources/emojify/images/sheet_emoji_64.png';
...
//        return val ? emoji.emojiarea_replacement(val, idx, ':') : "";
        ...
        emoji.init_unified();
        return idx.replace(emoji.rx_unified, function (m) {
            var val = emoji.map.unified[m];
            return val ? emoji.emojiarea_replacement(val) : "";
        });
    };
    
    emoji.emojiarea_get_alt = function (idx) {
        idx = idx.replace(/\:/g, "");
        emoji.init_colons();
        var val = emoji.map.colons[idx];
        return val ? emoji.data[val][0] : "";
    };

    emoji.emojiarea_replacement = function (idx) {
        var px = emoji.data[idx][4];
        var py = emoji.data[idx][5];
        if (emoji.use_sheet && px != null && py != null) {
            var mul = 100 / (emoji.sheet_size - 1);
