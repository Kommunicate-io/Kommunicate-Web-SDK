...
(function($applozic, w, d) {
        ...
        _this.stoptimer = function() {
            ...
            _this.init = function() {
				...
				$applozic("#km-form-chat-login").submit(function(e) {
					...
					console.log("appId", MCK_APP_ID);
					// $applozic.fn.applozic('reInitialize',{userId:userId,appId:MCK_APP_ID});
					var options = {
						userId : userId,
						appId : MCK_APP_ID,
						onInit : loadChat
					}
					var request = $applozic.ajax({
						url : MCK_BASE_URL + "/rest/ws/register/v2/admin",
						type : "post",
						data : formData,
    ...
    var replyMsg = mckStorage.getMessageByKey(msgkey);
      ...
      return  mckStorage.getMessageByKey(msgkey) ;
            ...
            _this.getGroup = function(params) {
                ...
                groupInfo.metadata = (groupInfo.type ===10)?MCK_LABELS['kommunicateGroup.metadata']:MCK_LABELS['group.metadata'];
                var response = new Object();
                mckUtils.ajax({
                ...
                if (typeof msg.fileMeta === "object") {
                    if (msg.fileMeta.contentType.indexOf("image") !== -1) {
                        if (msg.fileMeta.contentType.indexOf("svg") !== -1) {
                            return '<a href="#" target="_self"  role="link" class="file-preview-link fancybox-media fancybox" data-type="' + msg.fileMeta.contentType + '" data-url="' + MCK_FILE_URL + FILE_PREVIEW_URL + msg.fileMeta.blobKey + '" data-name="' + msg.fileMeta.name + '"><img src="' + MCK_FILE_URL + FILE_PREVIEW_URL + msg.fileMeta.blobKey + '" area-hidden="true"></img></a>';
                            ...
                            return '<a href="#" target="_self"  role="link" class="file-preview-link fancybox-media fancybox" data-type="' + msg.fileMeta.contentType + '" data-url="' + MCK_FILE_URL + FILE_PREVIEW_URL + msg.fileMeta.blobKey + '" data-name="' + msg.fileMeta.name + '"><img src="' + msg.fileMeta.thumbnailUrl + '" area-hidden="true" ></img></a>';
                        }
 ...
 _this.audioRecoder = function(params) {
        ...
        function ALSubscriber() {
            
           var _this = this;
           var topic;
           var stompClient;
           
           _this.subscribe  = function(stompClient, topic, options) {
               _this.topic = topic;
               _this.stompClient = stompClient;
               stompClient.subscribe(topic, options);
               return _this;
           };
           
           _this.unsubscribe = function() {
               _this.stompClient.unsubscribe(_this.topic);
           };
        }
        ...
        function MckInitializeChannel($this) {
            ...
            _this.init = function() {
                if (typeof MCK_WEBSOCKET_URL !== 'undefined') {
                    ...
                    //if (typeof w.SockJS === 'function') {
                        ...
                        stompClient = new Paho.MQTT.Client(MCK_WEBSOCKET_URL.replace("https://", "").replace("http://", ""), Number(15677), "/ws");
                        stompClient.onMessageArrived = _this.onMessage;
                        ...
                        //Todo: temp commented out, resolve it.
                        //stompClient.onConnectionLost = _this.disconnect();
                        
                        //stompClient.connect("guest", "guest", _this.onConnect, _this.onError, '/');
                        
                        var lastWillMessage = new Paho.MQTT.Message(MCK_TOKEN + "," + USER_DEVICE_KEY + ",0");
                        lastWillMessage.destinationName = "status-v2";
                        stompClient.connect({userName: 'guest', password: 'guest',onSuccess: _this.onConnect, willMessage: lastWillMessage, useSSL: true});
            ...
            _this.subscibeToTypingChannel = function(tabId, isGroup) {
                ...
                if (stompClient && stompClient.isConnected()) {
                    //typingSubscriber = stompClient.subscribe("typing-" + MCK_APP_ID + "-" + subscribeId, _this.onTypingStatus);
                    typingSubscriber = new ALSubscriber();
                    typingSubscriber.subscribe(stompClient, "typing-" + MCK_APP_ID + "-" + subscribeId);
                } else {
                    _this.reconnect();
                }
            };
            _this.subscribeToOpenGroup = function(group) {
                if (stompClient && stompClient.isConnected()) {
                    //var subs = stompClient.subscribe("group-" + MCK_APP_ID + "-" + group.contactId, _this.onOpenGroupMessage);
                    var subs = new ALSubscriber();
                    subs.subscribe(stompClient, "group-" + MCK_APP_ID + "-" + group.contactId);
            ...
            _this.sendTypingStatus = function(status, tabId) {
                if (stompClient && stompClient.isConnected()) {
                    if (status === 1 && MCK_TYPING_STATUS === 1) {
                        stompClient.send('typing-' + MCK_APP_ID + "-" + TYPING_TAB_ID, MCK_APP_ID + "," + MCK_USER_ID + "," + status);
                    }
                    if (tabId) {
                        ...
                        TYPING_TAB_ID = tabId;
                        stompClient.send('typing-' + MCK_APP_ID + "-" + tabId, MCK_APP_ID + "," + MCK_USER_ID + "," + status);
                        setTimeout(function() {
                            MCK_TYPING_STATUS = 0;
                        }, 60000);
                    } else if (status === 0) {
            ...
            _this.onTypingStatus = function(resp) {
                //if (typingSubscriber != null && typingSubscriber.id === resp.headers.subscription) {
                    var message = resp.payloadString;
            ...
            _this.sendStatus = function(status) {
                if (stompClient && stompClient.isConnected()) {
                    stompClient.send('status-v2',MCK_TOKEN + "," + USER_DEVICE_KEY + "," + status);
                }
            };
            _this.onConnect = function() {
                if (stompClient.isConnected()) {
                    ...
                    //subscriber = stompClient.subscribe("" + MCK_TOKEN, _this.onMessage);
                    subscriber = new ALSubscriber();
                    subscriber.subscribe(stompClient, "" + MCK_TOKEN);
                    _this.sendStatus(1);
                    _this.checkConnected(true);
                } else {
                    //Todo: temporarily removing timer.
                    /*setTimeout(function() {
                        //subscriber = stompClient.subscribe("" + MCK_TOKEN, _this.onMessage);
                        subscriber = new ALSubscriber();
                        subscriber.subscribe(stompClient, "" + MCK_TOKEN, {onSuccess: _this.onMessage});
                        _this.sendStatus(1);
                        _this.checkConnected(true);
                    }, 5000);*/
                }
            ...
            _this.onOpenGroupMessage = function(obj) {
                if (openGroupSubscriber.indexOf(obj.headers.subscription) !== -1) {
                    ...
                    var resp = $applozic.parseJSON(obj.payloadString);
            ...
            _this.onMessage = function(obj) {
                console.log(obj);
                if (obj.destinationName !== null && obj.destinationName.startsWith("typing-")) {
                    _this.onTypingStatus(obj);
                    return;
                } else if (obj.destinationName.startsWith("group-")) {
                    _this.onOpenGroupMessage(obj);
                    return;
                }
                //Todo: get topic based on obj.destinationName
