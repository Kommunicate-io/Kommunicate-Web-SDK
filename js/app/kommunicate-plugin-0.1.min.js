...
"undefined"!==typeof window.$applozic&&"undefined"!==typeof window.$applozic.fn&&"undefined"!==typeof window.$applozic.fn.applozic&&window.$applozic.fn.applozic("getLoggedInUser")&&window.$applozic.fn.applozic("logout");sessionStorage.clear();localStorage.clear()},launchConversation:function(){window.$applozic.fn.applozic("mckLaunchSideboxChat")},triggerEvent:function(a,f){$applozic.ajax({url:Kommunicate.getBaseUrl()+"/applications/events?type\x3d"+a,type:"post",data:JSON.stringify({conversationId:f.groupId,
applicationId:f.applicationId}),contentType:"application/json",success:function(a){console.log("conversation triggering event")},error:function(a){console.log("err while starting Conversation")}})},updateUser:function(a){window.$applozic.fn.applozic("updateUser",{data:a})},getAwayMessage:function(a,f){$applozic.ajax({url:Kommunicate.getBaseUrl()+"/applications/"+a.applicationId+"/awaymessage?conversationId\x3d"+a.conversationId,type:"get",contentType:"application/json",success:function(a){console.debug("got away message data");
"function"==typeof f?f(null,a):""},error:function(a){console.log("err while fetching away message");"function"==typeof f?f(a):""}})},updateUserIdentity:function(a){window.$applozic.fn.applozic("updateUserIdentity",{newUserId:a,callback:function(f){KommunicateUtils.setCookie("kommunicate-id",a);"success"==f&&window.$applozic.fn.applozic("reInitialize",{userId:a})}})},isRichTextMessage:function(a){return a&&300==a.contentType},getConatainerTypeForRichMessage:function(a){if(a)switch(a.templateId){case "2":case "4":return"km-slick-container";
case "6":return"km-border-less-container";default:return"km-fixed-container"}},processPaymentRequest:function(a){},sendMessage:function(a){var f=$applozic("#mck-message-cell .mck-message-inner"),c=$applozic("#mck-msg-to");!0===f.data("isgroup")?a.groupId=c.val():a.to=c.val();f=Kommunicate.getSettings("KM_CHAT_CONTEXT");c=a.metadata||{};f&&(c={KM_CHAT_CONTEXT:f});a.metadata=c;$applozic.fn.applozic("sendGroupMessage",a)},getRichTextMessageTemplate:function(a){if(a)switch(a.templateId){case "1":return Kommunicate.markup.getHotelRoomPaxInfoTemplate();
case "2":return Kommunicate.markup.getHotelCardContainerTemplate(JSON.parse(a.hotelList||"[]"),a.sessionId);case "3":return Kommunicate.markup.buttonContainerTemplate(a);case "5":return Kommunicate.markup.getPassangerDetail(a);case "4":return Kommunicate.markup.getRoomDetailsContainerTemplate(JSON.parse(a.hotelRoomDetail||"[]"),a.sessionId);case "6":return Kommunicate.markup.quickRepliesContainerTemplate(a);case "7":return Kommunicate.markup.getListContainerMarkup(a);case "8":return Kommunicate.markup.getDialogboxContainer(a);
default:return""}else return""},updateSettings:function(a){var f=KommunicateUtils.getDataFromKmSession("settings");f=f?JSON.parse(f):{};for(var c in a)f[c]=a[c];KommunicateUtils.storeDataIntoKmSession("settings",JSON.stringify(f))},getSettings:function(a){var f=KommunicateUtils.getDataFromKmSession("settings");f=f?JSON.parse(f):null;return a&&f?f[a]:f?f:""}});
...
g.push(b)});if(0<g.length)b.isMessages=u,b.messageData=l,b.isLoadMessageList=!0,ea.getUsersDetail(g,b);else{w.addGroupStatus(f);c=!0;var h;7===f.type&&(oa.removeClass("vis").addClass("n-vis"),y.getContactFromGroupOfTwo(f,function(a){h=m.fetchContact(a)}),Ba.lastSeenOfGroupOfTwo(h.contactId));"kommunicate"==window.applozic.PRODUCT_ID&&(3!==f.users[C].role&&2!==f.users[C].role&&0!==f.users[C].role||oa.removeClass("vis").addClass("n-vis"));6===f.type&&(w.validateOpenGroupUser(f),c=y.isAppendOpenGroupContextMenu(f));
ha.removeClass("vis").addClass("n-vis");u?(L.removeClass("vis").addClass("n-vis"),m.processMessageList(l,!0,c),6!==f.type&&qa.removeClass("n-vis").addClass("vis")):0===a("#mck-message-cell .mck-message-inner div[name\x3d'message']").length&&(qa.removeClass("vis").addClass("n-vis"),L.removeClass("n-vis").addClass("vis"));"function"===typeof kb&&kb(b.tabId)}}})}else b.startTime||(f.MCK_OL_MAP=[]),0<l.userDetails.length&&a.each(l.userDetails,function(a,b){ya[b.userId]=b;b.connected?f.MCK_OL_MAP[b.userId]=
!0:(f.MCK_OL_MAP[b.userId]=!1,"undefined"!==typeof b.lastSeenAtTime&&(ca[b.userId]=b.lastSeenAtTime));m.updateUnreadCount("user_"+b.userId,b.unreadCount,!1);a=m.getContact(""+b.userId);"undefined"===typeof a?m.createContactWithDetail(b):m.updateContactDetail(a,b)}),0<l.groupFeeds.length&&a.each(l.groupFeeds,function(a,b){m.updateUnreadCount("group_"+b.id,b.unreadCount,!1);D.addGroup(b)}),0<l.blockedUserPxyList.blockedToUserList.length&&a.each(l.blockedUserPxyList.blockedToUserList,function(a,b){b.userBlocked&&
(ta[b.blockedTo]=!0)}),0<l.blockedUserPxyList.blockedByUserList.length&&a.each(l.blockedUserPxyList.blockedByUserList,function(a,b){b.userBlocked&&(La[b.blockedBy]=!0)}),0<l.conversationPxys.length&&a.each(l.conversationPxys,function(b,c){da[c.id]=c;za[c.topicId]=[c.id];if(c.topicDetail)try{ba[c.topicId]=a.parseJSON(c.topicDetail)}catch(cc){f.console.log("Incorect Topic Detail!")}}),u?(b.startTime?b.startTime&&(b.isReload=!1,g||ALStorage.updateLatestMessageArray(l.message)):(b.isReload=!0,g||ALStorage.setLatestMessageArray(l.message)),
g&&ALStorage.updateMckMessageArray(l.message),m.addContactsFromMessageList(l,b)):q.data("datetime",""),b.isLaunch&&m.updateUnreadCountonChatIcon(l.userDetails)}t=n.data("forwardMessageKey");"undefined"!==typeof t&&(A.sendForwardMessage(t),n.data("forwardMessageKey",""))},error:function(a,b,c){401===a.status&&(sessionStorage.clear(),console.log("Please reload page."));Ka=!1;ha.removeClass("vis").addClass("n-vis");f.console.log("Unable to load messages. Please reload page.")}})};b.updateContactList=
function(a,f){var c={};f?c.groupId=a:c.userId=a;c.pageSize=1;window.Applozic.ALApiService.getMessages({data:c,success:function(c){c=c.data;"null"===c+""||"undefined"===typeof c.message||0===c.message.length?m.clearContactMessageData(a,f):(c=c.message[0],"undefined"!==typeof c&&(c.groupId?y.addGroupFromMessage(c,!0,function(a,f,c){b.updateRecentConversationList(a,f,c)}):m.addContactsFromMessage(c,!0)))},error:function(b){m.clearContactMessageData(a,f)}})};b.conversationReadUpdate=function(a){var b=
...
Fa[b.tabId]=g}m.loadTab(b);return}b.topicId&&(c={topicId:b.topicId,userId:b.tabId,status:b.topicStatus},b.isGroup?c.groupId=b.groupId:c.userId=b.tabId,g=ba[b.topicId],"object"===typeof g&&(c.topicDetail=f.JSON.stringify(g)),b.fallBackTemplatesList&&0<b.fallBackTemplatesList.length&&(c.fallBackTemplatesList=b.fallBackTemplatesList),window.Applozic.ALApiService.getConversationId({data:c,success:function(c){if("object"===typeof c&&"success"===c.status){var g=c.response;if("object"===typeof g&&"undefined"!==
g.conversationPxy){c=g.conversationPxy;da[c.id]=c;za[c.topicId]=[c.id];if(c.topicDetail)try{ba[c.topicId]=a.parseJSON(c.topicDetail)}catch(ob){f.console.log("Incorect Topic Detail!")}q.data("mck-conversationid",c.id);b.conversationId=c.id;if("undefined"!==typeof Fa[b.tabId]){var k=Fa[b.tabId];k.push(c);Fa[b.tabId]=k}b.isGroup&&(g=D.addGroup(g),b.tabId=g.contactId);b.isMessage&&c.created?m.loadTab(b,J.dispatchMessage):m.loadTab(b)}}},error:function(){}}))};b.sendPriceMessage=function(){var f=ra.val();
if(""===f)ra.addClass("mck-text-req");else{f=a.trim(f);var c=k.val();var g=q.data("mck-conversationid",g);var l={type:5,contentType:4,message:f};!0===q.data("isgroup")?l.groupId=c:l.to=c;if(q.data("mck-conversationid")){g=q.data("mck-conversationid");l.conversationId=g;var u=da[g];"object"!==u?J.getTopicId({conversationId:g,suppId:c,priceText:f},function(a){m.populateMessage(a.messageType,a.message,a.notifyUser)}):"function"===typeof Za&&Za({custId:C,suppId:c,productId:u.topicId,price:f});ra.val("")}b.sendMessage(l)}};
b.getGroup=function(b){var c=[];a.each(b.users,function(a,b){"undefined"!==typeof b.userId&&("undefined"!==typeof b.groupRole&&-1===Kb.indexOf(b.groupRole)||c.push(b))});var g={groupName:a.trim(b.groupName),users:c,type:b.type};b.admin&&(g.admin=b.admin);b.clientGroupId&&(g.clientGroupId=b.clientGroupId);b.groupIcon&&(g.imageUrl=b.groupIcon);g.metadata=b.metadata?b.metadata:MCK_LABELS["group.metadata"];var k={},l={"UserId-Enabled":!0,Authorization:"Basic "+p,"Application-Key":Z,"Device-Key":ka,"Access-Token":N};
P.ajax({url:MCK_BASE_URL+"/rest/ws/group/v2.1/create",global:!1,data:f.JSON.stringify(g),type:"post",headers:l,contentType:"application/json",success:function(c){b.isInternal&&Da.attr("disabled",!1).html(MCK_LABELS["create.group.title"]);if("object"===typeof c&&"success"===c.status){var g=c.response;if("object"===typeof g){var l=D.addGroup(g);0<g.users.length&&a.each(g.users,function(a,b){ya[b.userId]=b;b.connected?f.MCK_OL_MAP[b.userId]=!0:(f.MCK_OL_MAP[b.userId]=!1,"undefined"!==typeof b.lastSeenAtTime&&
(ca[b.userId]=b.lastSeenAtTime));m.updateUnreadCount("user_"+b.userId,b.unreadCount,!1);a=m.getContact(""+b.userId);"undefined"===typeof a?m.createContactWithDetail(b):m.updateContactDetail(a,b)});b.tabId=l.contactId;b.isGroup=!0;b.isMessage?m.loadTab(b,J.dispatchMessage):b.isInternal?(q.data("mck-id",l.contactId),q.data("isgroup",!0),w.loadGroupInfo({groupId:l.contactId})):m.loadTab(b);"function"===typeof b.callback&&(k.updated=c.updated?c.updated:!1,k.status="success",k.data=l,b.callback(k))}}else"error"===
c.status&&"function"===typeof b.callback&&(k.status="error",k.errorMessage=c.errorResponse[0].description,b.callback(k))},error:function(){b.isInternal&&Da.attr("disabled",!1).html(MCK_LABELS["create.group.title"]);"function"===typeof b.callback&&(k.status="error",k.errorMessage="Unable to process request.",b.callback(k))}})}},ea=new function(){a("#mck-search-list");var b=a("#mck-sidebox-search"),c=a("#mck-search-loading");a("#mck-search-cell .mck-message-inner");a("#mck-message-cell .mck-message-inner");
a(".mck-tab-message-option");a("#mck-contact-loading");this.getContactDisplayName=function(a){var b=[];window.Applozic.ALApiService.getContactDisplayName({data:{userIdArray:a},success:function(a){for(var f in a)a.hasOwnProperty(f)&&(b.push([f,a[f]]),Ea[f]=a[f],m.fetchContact(f).displayName=a[f]);ALStorage.updateMckContactNameArray(b)},error:function(){}})};this.loadContacts=function(){var g=[];window.Applozic.ALApiService.getContactList({url:"?startIndex\x3d0\x26pageSize\x3d50\x26orderBy\x3d1",baseUrl:MCK_BASE_URL,
success:function(c){b.hasClass("vis")&&("object"===typeof c&&0<c.users.length&&(a.each(c.users,function(a,b){"undefined"!==typeof b.userId&&(a=m.getContact(""+b.userId),a="undefined"===typeof a?m.createContactWithDetail(b):m.updateContactDetail(a,b),bb.push(a),g.push([b.userId,a.displayName]),b.connected?f.MCK_OL_MAP[b.userId]=!0:(f.MCK_OL_MAP[b.userId]=!1,"undefined"!==typeof b.lastSeenAtTime&&(ca[b.userId]=b.lastSeenAtTime)))}),ea.getUsersDetail(contactNameArray,{async:!1}),0<g.length&&ALStorage.updateMckContactNameArray(g)),
m.addContactsToSearchList(),w.addMembersToGroupSearchList())},error:function(){c.removeClass("vis").addClass("n-vis");f.console.log("Unable to load contacts. Please reload page.")}})};this.getUsersDetail=function(b,c){if(!("undefined"===typeof b||1>b.length)){for(var g="undefined"!==typeof c.cached?c.cached:!0,k=[],l=b.filter(function(a,f){return b.indexOf(a)===f}),n=0;n<l.length;n++){var x=l[n];g&&"undefined"!==typeof ya[x]||k.push(x)}if(0===k.length)c.setStatus?Ba.updateUserConnectedStatus():c.message?
m.populateMessage(c.messageType,c.message,c.notifyUser):c.isLoadMessageList&&m.loadMessageListOnUserDetailFetch(c);else{var p={};window.Applozic.ALApiService.getUserDetail({data:{userIdList:k},success:function(b){"success"===b.status&&0<b.response.length&&a.each(b.response,function(a,b){ya[b.userId]=b;f.MCK_OL_MAP[b.userId]=b.connected;a=m.getContact(""+b.userId);"undefined"===typeof a?m.createContactWithDetail(b):m.updateContactDetail(a,b)});c.setStatus?Ba.updateUserConnectedStatus():c.message?m.populateMessage(c.messageType,
c.message,c.notifyUser):c.isLoadMessageList&&m.loadMessageListOnUserDetailFetch(c);p.status="success";p.data=b;c.callback&&c.callback(p)},error:function(){c.setStatus?Ba.updateUserConnectedStatus():c.message?m.populateMessage(c.messageType,c.message,c.notifyUser):c.isLoadMessageList&&m.loadMessageListOnUserDetailFetch(c);p.status="error";c.callback&&c.callback(p)}})}}};this.updateUserIdentity=function(a){if(null!==a.newUserId&&""!==a.newUserId){var b="newUserId\x3d"+encodeURIComponent(a.newUserId);
P.ajax({url:MCK_BASE_URL+"/rest/ws/user/change/identifier",type:"get",data:b,contentType:"application/json",success:function(b){"object"===typeof b&&"success"===b.status&&a.callback&&a.callback(b)},error:function(b){a.callback&&a.callback({response:"error"})}})}};this.updateUser=function(a){P.ajax({type:"POST",url:MCK_BASE_URL+"/rest/ws/user/update",data:f.JSON.stringify(a.data),contentType:"application/json",success:function(b){a.success&&a.success(b)},error:function(b){a.error&&a.error(b)}})};this.updateDisplayName=
function(b,f){""!==b&&""!==f&&a.ajax({url:MCK_BASE_URL+"/rest/ws/user/name",type:"get",data:"userId\x3d"+b+"\x26displayName\x3d"+f,success:function(a){if("object"===typeof a&&"success"===a.status)return"success"},error:function(){}})}},Cb=new function(){var b=this,f,c,g,k,n,p,r;b.init=function(){a("#mck-sidebox");c=a("#mck-msg-preview");n=a("#mck-group-info-tab");a("#mck-sidebox-launcher");f=a("#mck-message-cell .mck-message-inner");g=a("#mck-msg-preview .mck-preview-icon");k=a("#mck-msg-preview .mck-preview-cont-name");
p=a("#mck-msg-preview .mck-preview-msg-content");r=a("#mck-msg-preview .mck-preview-file-content")};b.notifyUser=function(a){if(!1!==a.alert&&7!==a.type&&!1!==a.metadata.alert){var f=a.groupId?D.getGroup(""+a.groupId):m.fetchContact(""+a.to.split(",")[0]),c=!1;a.groupId&&(c=!0);va.loadUserProfile(a.to);c=m.getTabDisplayName(f.contactId,c);var g=yc;b.showNewMessageNotification(a,f,c);if(qc&&!Hb){var k=Cc;a=m.getTextForMessagePreview(a,f);"function"!==typeof $a||f.isGroup||(f=$a(f.contactId))&&"undefined"!==
typeof f&&(k=f);g?zc.sendDesktopNotification(c,k,a,g):zc.sendDesktopNotification(c,k,a)}}};b.showNewMessageNotification=function(a,b,l){if(rc&&102!==a.contentType){var x=f.data("mck-id"),W=f.data("isgroup");if(x===b.contactId&&W===b.isGroup&&!n.hasClass("vis"))if(a.conversationId&&(Oa||ab)){if(x=f.data("mck-conversationid"),x="undefined"!==typeof x&&""!==x?x.toString():"",x===a.conversationId.toString())return}else return;c.data("isgroup",b.isGroup);c.data("mck-conversationid",a.conversationId?a.conversationId:
"");x=m.getContactImageLink(b,l);a.message?(W=m.getMessageTextForContactPreview(a,b,100),p.html(""),"object"===typeof W?p.append(W):p.html(W),p.removeClass("n-vis").addClass("vis")):p.html("");a.fileMetaKey?(r.html(I.getFileIcon(a)),r.removeClass("n-vis").addClass("vis"),""===p.html()&&p.removeClass("vis").addClass("n-vis")):(r.html(""),r.removeClass("vis").addClass("n-vis"));!0===b.isGroup&&10!==b.type&&k.html(l);g.html(x);c.data("mck-id",b.contactId);c.show();setTimeout(function(){c.fadeOut(3E3)},
1E4)}}},J=new AlMessageService,I=new AlFileService,zc=new MckNotificationUtils,vc=new AlNotificationService,va=new AlUserService,Qa=a(".chat-launcher-icon"),la=new function(){var b=this,f;b.token=null;b.Identity=null;b.outgoingCallServices=null;b.incomingCallServices=null;var c=a(".mck-videocall-btn"),g=a(".applozic-vid-container"),k=a("#mck-sidebox"),n=a("#mck-video-call-indicator"),p=a("#mck-video-box");b.hideVideoBox=function(){g.addClass("n-vis").removeClass("vis");k.addClass("vis").removeClass("n-vis");
n.addClass("n-vis").removeClass("vis")};b.init=function(){p.mckModal();wa.loop=!0;b.token=null!==ALStorage.getAppHeaders()?ALStorage.getAppHeaders().videoToken:void 0;b.ringToneForHost=qb.loadRingTone(Kommunicate.BASE_URL[MCK_BASE_URL]+"/plugin/audio/applozic_video_call_ring_tone.mp3",wa);a("#mck-btn-video-call").on("click",function(c){if(b.token){var g=$mck_msg_to.val();c=C+(new Date).getTime().toString()+Math.random().toString(36).slice(2);g=J.sendVideoCallMessage(c,"CALL_DIALED",102,!1,g,function(a){A.sendMessage(a)});
var k=m.fetchContact(g.to);g=m.getTabDisplayName(k.contactId,!1);k=m.getContactImageLink(k,g);var l=new Date;b.outgoingCallServices=new MckCallingService(C,la.token,c,g,!0,l,A,k,!1,b.ringToneForHost);b.outgoingCallServices.startVideoCall();a("#mck-btn-video-call").data("isCallHost",!0);a("#mck-btn-video-call").data("callStartTime",l)}else"INSUFFICIENT_FUNDS"===f?alert("Video calling is not available at the moment. For details, contact support."):alert("missing token... please refresh page..")});a("#mck-vid-receive-btn").on("click",
function(b){console.log("call received");$("#mck-video-call-indicator").addClass("n-vis").removeClass("vis");c.removeClass("vis").addClass("n-vis");b=a("#mck-video-call-indicator").data("call-id");var f=a("#mck-video-call-indicator").data("isAudioCall");a("#mck-video-call-indicator").data("callReceived",!0);a("#mck-video-call-indicator").addClass("n-vis");la.callReceived(b,f)});a("#mck-vid-reject-btn").on("click",function(b){console.log("call rejected");b=a("#mck-video-call-indicator").data("call-id");
var f=a("#mck-video-call-indicator").data("isAudioCall");la.callRejected(b,f);a("#mck-video-call-indicator").addClass("n-vis").removeClass("vis")})};b.InitilizeVideoClient=function(c,g){b.Identity=c;a.ajax({url:MCK_BASE_URL+"/twilio/token",type:"post",contentType:"application/x-www-form-urlencoded",data:{identity:c,device:g},success:function(a){if(null!=a&&""!=a){b.token=a.token;var c=ALStorage.getAppHeaders();c.videoToken=a.token;ALStorage.setAppHeaders(c)}a.hasOwnProperty("errorResponse")&&(f=a.errorResponse[0].description)},
error:function(a){console.log("error while getting token"+a)}})};b.callReceived=function(a,f){var c=$mck_msg_to.val();console.log("_this.callReceived");xa&&xa.stop();J.sendVideoCallMessage(a,"CALL_ANSWERED",102,f,c,function(a){A.sendMessage(a)});b.incomingCallServices=new MckCallingService(b.Identity,b.token,a,null,!1,null,A,null,f);b.incomingCallServices.startVideoCall()};b.callRejected=function(a,b){xa&&xa.stop();J.sendVideoCallMessage(a,"CALL_REJECTED",102,b,toUser,function(a){A.sendMessage(a)})}},
yc=null,wa={},qb,Hc=!1,xa=null,Yb=g.isAnonymousChat,Ga=g.askUserDetails,fb=g.conversationTitle,gb=g.agentId,hb=g.botIds;f.MCK_OL_MAP=[];k.mckLaunchSideboxChat=function(){a("#mck-sidebox-launcher").removeClass("vis").addClass("n-vis");KommunicateUI.showChat();a("#mck-away-msg-box").removeClass("vis").addClass("n-vis");A.loadConversationWithAgents({groupName:fb,agentId:gb,botIds:hb},function(){console.log("conversation created successfully")})};k.events={onConnectFailed:function(){},onConnect:function(){},
...
p.disconnect(),H.close(),H="")};c.unsubscibeToTypingChannel=function(){p&&p.connected&&v&&(1===na&&c.sendTypingStatus(0,r),v.unsubscribe());v=null};c.unsubscibeToNotification=function(){p&&p.connected&&k&&k.unsubscribe();k=null};c.subscibeToTypingChannel=function(a,b){a=b?a:C;p&&p.connected?v=p.subscribe("/topic/typing-"+Z+"-"+a,c.onTypingStatus):c.reconnect()};c.subscribeToOpenGroup=function(a){if(p&&p.connected){var b=p.subscribe("/topic/group-"+Z+"-"+a.contactId,c.onOpenGroupMessage);I.push(b.id);
Jb[a.contactId]=b.id}else c.reconnect()};c.sendTypingStatus=function(a,b){if(p&&p.connected){1===a&&1===na&&p.send("/topic/typing-"+Z+"-"+r,{"content-type":"text/plain"},Z+","+C+","+a);if(b){if(b===r&&a===na&&1===a)return;r=b;p.send("/topic/typing-"+Z+"-"+b,{"content-type":"text/plain"},Z+","+C+","+a);setTimeout(function(){na=0},6E4)}else 0===a&&p.send("/topic/typing-"+Z+"-"+r,{"content-type":"text/plain"},Z+","+C+","+a);na=a}};c.onTypingStatus=function(a){if(null!=v&&v.id===a.headers.subscription){var b=
...
Y.html(c+" "+MCK_LABELS["is.typing"]);7===k.type&&(N.addClass("mck-tab-title-w-typing"),Y.html(MCK_LABELS["is.typing"]),R.html(""))}}else N.addClass("mck-tab-title-w-typing"),R.removeClass("vis").addClass("n-vis"),Y.html(MCK_LABELS.typing);U.removeClass("n-vis").addClass("vis");setTimeout(function(){N.removeClass("mck-tab-title-w-typing");U.removeClass("vis").addClass("n-vis");N.hasClass("undefined"===typeof k||7!=k.type)&&R.removeClass("n-vis").addClass("vis");Y.html(MCK_LABELS.typing)},6E4)}}else N.removeClass("mck-tab-title-w-typing"),
U.removeClass("vis").addClass("n-vis"),!N.hasClass("mck-tab-title-w-status")||"undefined"!==typeof k&&7==k.type||R.removeClass("n-vis").addClass("vis"),Y.html(MCK_LABELS.typing)}};c.reconnect=function(){c.unsubscibeToTypingChannel();c.unsubscibeToNotification();c.disconnect();c.init()};c.onError=function(a){f.console.log("Error in channel notification. "+a);g.onConnectFailed()};c.sendStatus=function(a){p&&p.connected&&p.send("/topic/status-v2",{"content-type":"text/plain"},n+","+ka+","+a)};c.onConnect=
...
l,0,!0),p={userId:l},b&&(p.topicId=b),g.onConversationDeleted(p));else if("APPLOZIC_11"===c){l=b.message;p=m.fetchContact(l);n=Q.data("mck-id");if(!ta[l]&&!La[l]){if(n!==p.contactId||Q.data("isgroup")){var r=mckContactUtils.formatContactId(l);a("#li-user-"+r+" .mck-ol-status").removeClass("n-vis").addClass("vis")}else a("#mck-tab-status").html(MCK_LABELS.online),ub&&m.hideOfflineMessage();a(".mck-user-ol-status."+r).removeClass("n-vis").addClass("vis");a(".mck-user-ol-status."+r).next().html("("+
MCK_LABELS.online+")");f.MCK_OL_MAP[l]=!0;va.updateUserStatus({userId:b.message,status:1},function(a){ea.getUsersDetail(a,{})})}g.onUserConnect({userId:b.message})}else if("APPLOZIC_12"===c)l=b.message.split(",")[0],b=b.message.split(",")[1],p=m.fetchContact(l),f.MCK_OL_MAP[l]=!1,b&&(ca[l]=b),ta[l]||La[l]||(n=Q.data("mck-id"),n!==p.contactId||Q.data("isgroup")||(a("#mck-tab-status").html(mckDateUtils.getLastSeenAtStatus(b)),ub&&mb.manageOfflineMessageTime(n)),a(".mck-user-ol-status."+p.htmlId).removeClass("vis").addClass("n-vis"),
a(".mck-user-ol-status."+p.htmlId).next().html("(Offline)"),a("#li-user-"+r+" .mck-ol-status").removeClass("vis").addClass("n-vis"),va.updateUserStatus({userId:l,status:0,lastSeenAtTime:b},function(a){ea.getUsersDetail(a,{})})),g.onUserDisconnect({userId:l,lastSeenAtTime:b});else if("APPLOZIC_29"===c){l=b.message.split(",")[0];b=b.message.split(",")[1];p=m.fetchContact(l);m.updateUnreadCount("user_"+p.contactId,0,!0);n=Q.data("mck-id");if("undefined"===typeof n||""===n)a("#li-user-"+p.htmlId+" .mck-unread-count-text").html(m.getUnreadCount("user_"+
p.contactId)),a("#li-user-"+p.htmlId+" .mck-unread-count-box").removeClass("vis").addClass("n-vis");p={userId:l};b&&(p.topicId=b);g.onConversationReadFromOtherSource(p)}else if("APPLOZIC_28"===c)l=b.message.split(",")[0],b=b.message.split(",")[1],n=Q.data("mck-id"),n===l&&(a(".mck-msg-right .mck-message-status").removeClass("mck-icon-time").removeClass("mck-icon-sent").removeClass("mck-icon-delivered").addClass("mck-icon-read"),a(".mck-msg-right .mck-icon-delivered").attr("title","delivered and read"),
p=m.getContact(l),"undefined"===typeof p&&(p=[],p.push(l),ea.getUsersDetail(p,{}))),p={userId:l},b&&(p.topicId=b),g.onConversationRead(p);else if("APPLOZIC_16"===c)c=b.message.split(":")[0],l=b.message.split(":")[1],p=m.fetchContact(l),n=Q.data("mck-id"),n===p.contactId?c===uc[0]?(ta[p.contactId]=!0,Ba.toggleBlockUser(n,!0)):(La[p.contactId]=!0,N.removeClass("mck-tab-title-w-status"),R.removeClass("vis").addClass("n-vis"),U.removeClass("vis").addClass("n-vis")):a("#li-user-"+p.htmlId+" .mck-ol-status").removeClass("vis").addClass("n-vis"),
g.onUserBlocked({status:c,userId:l});else if("APPLOZIC_17"===c){c=b.message.split(":")[0];l=b.message.split(":")[1];p=m.fetchContact(l);n=Q.data("mck-id");if(n===p.contactId)if(c===uc[2])ta[p.contactId]=!1,Ba.toggleBlockUser(n,!1);else{if(f.MCK_OL_MAP[n]||ca[n])La[p.contactId]=!1,ta[n]||(f.MCK_OL_MAP[n]?R.html(MCK_LABELS.online):ca[n]&&R.html(mckDateUtils.getLastSeenAtStatus(ca[n])),N.addClass("mck-tab-title-w-status"),R.removeClass("n-vis").addClass("vis"))}else f.MCK_OL_MAP[n]&&a("#li-user-"+p.htmlId+
" .mck-ol-status").removeClass("n-vis").addClass("vis");g.onUserUnblocked({status:c,userId:l})}else if("APPLOZIC_18"===c)tb=!1,g.onUserActivated();else if("APPLOZIC_19"===c)tb=!0,g.onUserDeactivated();else if(l=b.message,"APPLOZIC_03"===c)ALStorage.updateLatestMessage(l),0!==l.type&&4!==l.type&&(a("."+l.key+" .mck-message-status").removeClass("mck-icon-time").addClass("mck-icon-sent"),m.addTooltip(l.key)),g.onMessageSentUpdate({messageKey:l.key});else if("APPLOZIC_01"===c||"APPLOZIC_02"===c||"MESSAGE_RECEIVED"===
c)ALStorage.updateLatestMessage(l),p=l.groupId?D.getGroup(l.groupId):m.getContact(l.to),a("#mck-sidebox-content"),n=Q.data("mck-id"),"APPLOZIC_01"===c||"MESSAGE_RECEIVED"===c?(n=m.getMessageFeed(l),g.onMessageReceived({message:n})):"APPLOZIC_02"===c&&(n=m.getMessageFeed(l),g.onMessageSent({message:n})),l.conversationId&&(n=da[l.conversationId],!Oa&&!ab||"object"===typeof n&&"object"===typeof ba[n.topicId]||J.getTopicId({conversationId:l.conversationId,messageType:c,message:l,notifyUser:b.notifyUser,
...
l.metadata.CALL_AUDIO_ONLY),a("#mck-video-call-indicator-txt").html(n+" calling..."),a("#mck-video-call-icon").html(p),a("#mck-video-call-indicator").removeClass("n-vis").addClass("vis"),xa.play(),setTimeout(function(){a("#mck-video-call-indicator").data("callReceived")||(console.log("call is not answered"),xa.stop(),a("#mck-video-call-indicator").addClass("n-vis").removeClass("vis"))},6E4)):4==l.type&&"CALL_REJECTED"==l.metadata.MSG_TYPE&&a("#mck-btn-video-call").data("isCallHost")&&(p=$mck_msg_to.val(),
J.sendVideoCallMessage(l.metadata.CALL_ID,"CALL_REJECTED",103,!1,p,function(a){A.sendMessage(a)}),la.ringToneForHost.stop(),la.outgoingCallServices.twilioService.leaveRoomIfJoined(),la.hideVideoBox(),la.outgoingCallServices&&(la.outgoingCallServices.rejectedByReceiver=!0))),103==l.contentType?4==l.type&&"CALL_MISSED"==l.metadata.MSG_TYPE&&(a("#mck-video-call-indicator").addClass("n-vis").removeClass("vis"),xa&&xa.stop()):m.populateMessage(c,l,b.notifyUser))}}}(k);k.getOptions=function(){return g};
k.init=function(){window.Applozic.ALApiService.initServerUrl(MCK_BASE_URL);I.get(g);J.init(g);vc.init(g);m.init();wa.loop=!1;ic&&(qb=new RingToneService,yc=qb.loadRingTone(ic,wa));A.init();Fb.init();w.init();mb.initializeApp(g,!1);Cb.init();Zb.init();m.initEmojis();Sa&&(wa.loop=!0,qb=new RingToneService,xa=qb.loadRingTone(Kommunicate.BASE_URL[MCK_BASE_URL]+"/plugin/audio/applozic_video_call_ring_tone.mp3",wa),la.init())};k.reInit=function(b){KommunicateUtils.storeDataIntoKmSession("appOptions",b);
"object"===a.type(b)&&(b=a.extend(!0,{},r,b),g.conversationTitle=g.conversationTitle||g.groupName,g.accessToken=g.password||g.accessToken,a.fn.applozic("reset",b),b.userId&&b.appId&&""!==a.trim(b.userId)&&""!==a.trim(b.appId)?(mb.initializeApp(b,!0),g=b):f.console("Oops! looks like incorrect application id or user Id."))};k.loadTabView=function(b){"undefined"===typeof b.isGroup&&(b.isGroup=!1);m.loadTab(b);a("#mck-search").val("")};k.loadTab=function(b,c){m.loadTab({tabId:b,isGroup:!1},c);a("#mck-search").val("")};
k.loadChat=function(b){if(!("undefined"!==typeof b.userId&&""!==b.userId||"undefined"!==typeof b.groupId&&""!==b.groupId||"undefined"!==typeof b.clientGroupId&&""!==b.clientGroupId))return"UserId or groupId or clientGroupId required";var c={};b.userId&&(c.tabId=userId,c.isGroup=!1);b.displayName&&(c.userName=b.displayName);b.convId&&(c.conversationId=b.convId);if(b.groupId||b.clientGroupId)if(c.isGroup=!0,b.clientGroupId){c.clientGroupId=b.clientGroupId;var f=D.getGroupByClientGroupId(b.clientGroupId);
if("object"===typeof f)c.tabId=f.contactId;else{y.getGroupFeed({clientGroupId:c.clientGroupId,apzCallback:w.onGroupFeed,callback:w.loadGroupTab});a("#mck-search").val("");return}}else if(f=D.getGroup(b.groupId),"object"===typeof f)c.tabId=b.groupId;else{y.getGroupFeed({groupId:b.groupId,apzCallback:w.onGroupFeed,callback:w.loadGroupTab});a("#mck-search").val("");return}m.loadTab(c);a("#mck-search").val("")};k.uploadFile=function(a){Fb.uploadFile(a)};k.audioAttach=function(a){Fb.audioRecoder(a)};k.loadGroupTab=
function(b){if("undefined"===typeof b||""===b)return"GroupId required";"object"===typeof D.getGroup(b)?(m.loadTab({tabId:b,isGroup:!0}),a("#mck-search").val("")):y.getGroupFeed({groupId:b,apzCallback:w.onGroupFeed,callback:w.loadGroupTab})};k.loadGroupTabByClientGroupId=function(b){if("undefined"===typeof b.clientGroupId||""===b.clientGroupId)return"Client Group Id Required";var c=D.getGroupByClientGroupId(b.clientGroupId);"object"===typeof c?(m.loadTab({tabId:c.contactId,isGroup:!0}),a("#mck-search").val("")):
y.getGroupFeed({clientGroupId:b.clientGroupId,apzCallback:w.onGroupFeed,callback:w.loadGroupTab})};k.loadConvTab=function(a){"object"===typeof a&&a.userId&&a.convId&&m.loadTab({tabId:a.userId,conversationId:a.convId,isGroup:!1})};k.loadTabWithTopic=function(a){if("object"===typeof a&&(a.userId||a.groupId)&&a.topicId){var b={tabId:a.userId,topicId:a.topicId};a.userName&&(b.userName=a.userName);b.topicStatus=a.topicStatus?-1===R.indexOf(a.topicStatus)?R[0]:a.topicStatus.toString():R[0];if("function"===
typeof Ta||a.topicDetail){var c=a.topicDetail?a.topicDetail:Ta(a.topicId);"object"===typeof c&&"undefined"!==c.title&&(ba[a.topicId]=c)}a.message?(b.messagePxy={type:5,contentType:0,message:message},b.isMessage=!0):b.isMessage=!1;a.supportId?(b.isGroup=!0,b.supportId=supportId):(b.isGroup=!1,b.userId=a.userId);A.getConversationId(b)}else{if(!a.userId||!a.groupId)return"UserId required";if(!a.topicId)return"TopicId required"}};k.loadContacts=function(a){m.loadContacts(a)};k.loadContactsForContactList=
function(a){m.loadContactsForContactList(a)};k.setOffline=function(){"undefined"!==typeof fa&&fa.sendStatus(0)};k.reset=function(b){f.sessionStorage.clear();p=n="";O=[];MCK_GROUP_MAP=[];Gb=!0;Ja=[];ka="";Ib=b.mode;ta=[];La=[];Ka=!1;Da=90;Z=b.appId;ca=[];da=[];ba=[];MCK_CLIENT_GROUP_MAP=[];Hb=!0;MCK_LABELS=b.labels;X=0;MCK_BASE_URL=b.baseUrl;Na=b.fileupload;dc=b.customFileUrl;Y={};lb=[];rb=b.launcher;Jb=[];ec=b.userName;jb=b.visitor;za=[];bb=[];Ua={};Ma=b.fileBaseUrl;sb=b.locShare;Sa=g.video;U=b.onInit;
Mb=b.onTopicDetails;Ea=[];Pa=[];Nb=b.onClose;N=b.accessToken;Ob=b.displayText;kb=b.readConversation;oa=b.maxGroupSize;Fa=[];Pb=b.onTabClicked;fc=b.contactNumber;yb=b.appModuleName;Ta=b.getTopicDetail;Aa=b.maxAttachmentSize;Rb=b.validateMessage;Qb=b.contactDisplayName;cb=[];Za=b.finalPriceResponse;$a=b.contactDisplayImage;Sb=b.priceWidget;pa=g.openGroupSettings;ua=b.offlineMessageDetail;Tb=b.getConversationDetail;zb=b.authenticationTypeId;C=jb?"guest":a.trim(b.userId);jc=sb?b.googleApiKey:"NO_ACCESS";
Ab="boolean"===typeof b.olStatus?b.olStatus:!1;ab="boolean"===typeof b.topicBox?b.topicBox:!1;ub="boolean"===typeof b.showOfflineMessage?b.showOfflineMessage:!1;Oa="boolean"===typeof b.topicHeader?b.topicHeader:!1;rc="boolean"===typeof b.notification?b.notification:!0;pc=b.supportId?'data-mck-id\x3d"'+b.supportId+'"':"";lc="boolean"===typeof b.hideGroupSubtitle?b.hideGroupSubtitle:!1;mc="undefined"===typeof b.defaultMessageMetaData?{}:b.defaultMessageMetaData;Vb="boolean"===typeof b.groupUserCount?
b.groupUserCount:!1;qc="boolean"===typeof b.desktopNotification?b.desktopNotification:!1;Wb="boolean"===typeof b.loadOwnContacts?b.loadOwnContacts:!1;kc="boolean"===typeof b.messageBubbleAvator?b.messageBubbleAvator:!1;oc="boolean"===typeof g.resetUserStatus?g.resetUserStatus:!1;Bb="boolean"===typeof b.launchOnNewMessage?b.launchOnNewMessage:!1;sc="boolean"===typeof b.autoTypeSearchEnabled?b.autoTypeSearchEnabled:!0;nc="boolean"===typeof b.checkUserBusyWithStatus?b.checkUserBusyWithStatus:!1;Xb="boolean"===
typeof b.launchOnUnreadMessage?b.launchOnUnreadMessage:!1;Yb=g.isAnonymousChat;Ga=g.askUserDetails};k.logout=function(){"undefined"!==typeof fa&&(fa.disconnect(),ALStorage.clearMckMessageArray(),a.fn.applozic("reset",g),a("#mck-sidebox").hide(),a("#mck-sidebox-launcher").hide());Gb=!1};k.setOnline=function(){"undefined"!==typeof fa&&fa.sendStatus(1)};k.getUserStatus=function(b){"function"===typeof b.callback&&("undefined"!==typeof b.userIds?ea.getUsersDetail(b.userIds,b):va.getUserStatus(b,function(b){a.each(b.users,
function(a,b){a=m.getContact(""+b.userId);a="undefined"===typeof a?m.createContactWithDetail(b):m.updateContactDetail(a,b);cb.push(a.contactId)})}))};k.getGroupList=function(a){return"function"===typeof a.callback?(a.apzCallback=w.loadGroups,y.loadGroups(a),"success"):"Callback Function Required"};k.leaveGroup=function(a){if("object"!==typeof a)return"Unsupported Format. Please check format";if("function"===typeof a.callback){if("undefined"!==typeof a.groupId&&""!==a.groupId||"undefined"!==typeof a.clientGroupId&&
""!==a.clientGroupId)return a.apzCallback=w.onGroupLeft,y.leaveGroup(a),"success";a.callback({status:"error",errorMessage:"GroupId or Client GroupId Required"})}else return"Callback Function Required"};k.addGroupMember=function(a){if("object"!==typeof a)return"Unsupported Format. Please check format";if("function"===typeof a.callback)if("undefined"!==typeof a.groupId&&""!==a.groupId||"undefined"!==typeof a.clientGroupId&&""!==a.clientGroupId)if("undefined"===typeof a.userId||""===a.userId)a.callback({status:"error",
errorMessage:"UserId required"});else if("undefined"!==typeof a.role&&-1===Kb.indexOf(a.role))a.callback({status:"error",errorMessage:"Incorrect member role"});else return va.loadUserProfile(a.userId),a.apzCallback=w.onAddedGroupMember,y.addGroupMember(a),"success";else a.callback({status:"error",errorMessage:"GroupId or clientGroupId required"});else return"Callback function required"};k.removeGroupMember=function(a){if("object"!==typeof a)return"Unsupported Format. Please check format";if("function"===
...
KommunicateUI={awayMessageStatus:!1,leadCollectionStatus:!0,CONSTS:{},populateAwayMessage:function(a,f){var c=$applozic("#mck-tab-individual").hasClass("n-vis");!a&&"SUCCESS"==f.code&&0<f.data.messageList.length&&!c?(KommunicateUI.awayMessageStatus=!0,KommunicateUI.leadCollectionStatus=f.data.collectEmail,awayMessage=f.data.messageList[0].message,$applozic("#mck-away-msg").html(awayMessage),$applozic("#mck-away-msg-box").removeClass("n-vis").addClass("vis")):$applozic("#mck-away-msg-box").removeClass("vis").addClass("n-vis")},
hideAwayMessage:function(){$applozic("#mck-away-msg").html("");$applozic("#mck-away-msg-box").removeClass("vis").addClass("n-vis")},displayLeadCollectionTemplate:function(a){if(a&&a.length){for(var f=0,c=0;c<a.length&&(5!=a[c].type||(f++,2!=f));c++);1==f?KommunicateUI.leadCollectionStatus&&KommunicateUI.awayMessageStatus&&$applozic("#mck-email-collection-box").removeClass("n-vis").addClass("vis"):$applozic("#mck-email-collection-box").removeClass("vis").addClass("n-vis")}else null==a&&$applozic("#mck-email-collection-box").removeClass("n-vis").addClass("vis")},
hideLeadCollectionTemplate:function(){$applozic("#mck-email-collection-box").removeClass("vis").addClass("n-vis");$applozic("#mck-email-error-alert-box").removeClass("vis").addClass("n-vis")},validateEmail:function(a){if(a.match(/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/))return $applozic("#mck-email-error-alert-box").removeClass("vis").addClass("n-vis"),!0;$applozic("#mck-email-error-alert-box").removeClass("n-vis").addClass("vis");$applozic("#mck-email-collection-box").removeClass("vis").addClass("n-vis");
return!1},faqEvents:function(a,f){var c;$applozic(d).on("click","#mck-msg-preview",function(){KommunicateUI.showChat()});$applozic(d).on("click",".km-faq-list",function(){$applozic("#km-faqanswer").empty();var c=$(this).attr("data-articleid"),r=$(this).attr("data-source");KommunicateKB.getArticle({data:{appId:a.appId,articleId:c,source:r,helpdocsAccessKey:f},success:function(a){0==$applozic("#km-faqanswer .km-faqanswer-list").length&&($applozic("#km-faqanswer").append('\x3cdiv class\x3d"km-faqanswer-list km-faqanswerscroll"\x3e\x3cdiv class\x3d"km-faqquestion"\x3e'+
a.data.title+'\x3c/div\x3e \x3cdiv class\x3d"km-faqanchor km-faqanswer"\x3e'+a.data.body+"\x3c/div\x3e\x3c/div\x3e"),$applozic("#km-contact-search-input-box").removeClass("vis").addClass("n-vis"),$applozic("#km-faqdiv").removeClass("vis").addClass("n-vis"),$applozic("#km-faqanswer").removeClass("n-vis").addClass("vis"),$applozic("#mck-tab-individual").removeClass("n-vis").addClass("vis"),$applozic("#mck-tab-conversation").removeClass("vis").addClass("n-vis"),$applozic("#mck-no-conversations").removeClass("vis").addClass("n-vis"))},
error:function(){}})});$applozic(d).on("click","#km-faq",function(){KommunicateUI.showHeader();$applozic("#km-faq").removeClass("vis").addClass("n-vis");$applozic("#mck-no-conversations").removeClass("vis").addClass("n-vis");$applozic(".faq-common").removeClass("n-vis").addClass("vis");$applozic(".mck-conversation ").removeClass("vis").addClass("n-vis");$applozic("#km-faqdiv").removeClass("n-vis").addClass("vis");$applozic(".mck-conversation-back-btn").removeClass("n-vis").addClass("vis");$applozic("#mck-tab-title").html("FAQ").removeClass("n-vis").addClass("vis");
$applozic("#mck-away-msg-box").addClass("n-vis").removeClass("vis");$applozic("#mck-sidebox-ft").addClass("n-vis").removeClass("vis");$applozic("#mck-contacts-content").addClass("n-vis").removeClass("vis")});$applozic(d).on("click","#km-faqanswer a",function(a){a.preventDefault();window.open(a.target.href)});$applozic("#km-faq-search-input").keydown(function(n){clearTimeout(c);c=setTimeout(function(){KommunicateKB.getArticles({data:{appId:a.appId,query:document.getElementById("km-faq-search-input").value,
helpdocsAccessKey:f},success:function(a){a.data&&0===a.data.length&&$applozic(".km-no-results-found-container").hasClass("n-vis")?($applozic(".km-no-results-found-container").addClass("vis").removeClass("n-vis"),$applozic(".km-talk-to-human-div p").text("We are here to help.")):($applozic(".km-no-results-found-container").addClass("n-vis").removeClass("vis"),$applozic(".km-talk-to-human-div p").text("Looking for something else? "));$applozic(".km-talk-to-human-div").addClass("vis").removeClass("n-vis");
$applozic("#km-faqdiv").empty();$applozic.each(a.data,function(a,c){$applozic("#km-faqdiv").append('\x3cli class\x3d"km-faq-list" data-source\x3d"'+c.source+'" data-articleId\x3d"'+c.articleId+'"\x3e\x3ca class\x3d"km-faqdisplay"\x3e \x3cdiv\x3e\x3cdiv class\x3d"km-faqimage"\x3e\x3c/div\x3e\x3c/div\x3e \x3cdiv class\x3d"km-faqanchor"\x3e'+c.title+"\x3c/div\x3e\x3c/a\x3e\x3c/li\x3e")})},error:function(){}})},2E3);32!=n.which&&13!=n.which||KommunicateKB.getArticles({data:{appId:a.appId,query:document.getElementById("km-faq-search-input").value,
helpdocsAccessKey:f},success:function(a){a.data&&0===a.data.length&&$applozic(".km-no-results-found-container").hasClass("n-vis")?($applozic(".km-no-results-found-container").addClass("vis").removeClass("n-vis"),$applozic(".km-talk-to-human-div p").text("We are here to help. ")):($applozic(".km-no-results-found-container").addClass("n-vis").removeClass("vis"),$applozic(".km-talk-to-human-div p").text("Looking for something else? "));$applozic(".km-talk-to-human-div").addClass("vis").removeClass("n-vis");
$applozic("#km-faqdiv").empty();$applozic.each(a.data,function(a,c){$applozic("#km-faqdiv").append('\x3cli class\x3d"km-faq-list" data-source\x3d"'+c.source+'" data-articleId\x3d"'+c.articleId+'"\x3e\x3ca class\x3d"km-faqdisplay"\x3e \x3cdiv\x3e\x3cspan class\x3d"km-faqimage"/ \x3e\x3c/span\x3e \x3cdiv class\x3d"km-faqanchor "\x3e'+c.title+"\x3c/div\x3e\x3c/a\x3e\x3c/li\x3e")})},error:function(){}})});$applozic(d).on("click","#mck-conversation-back-btn",function(){$applozic(".km-kb-container").hasClass("vis")&&
($applozic("#km-faqanswer").hasClass("vis")?(KommunicateUI.showHeader(),$applozic("#km-faqdiv").removeClass("n-vis").addClass("vis"),$applozic("#km-faqanswer").removeClass("vis").addClass("n-vis"),$applozic("#km-contact-search-input-box").removeClass("n-vis").addClass("vis")):$applozic("#km-faqdiv").hasClass("vis")?($applozic("#mck-tab-individual").removeClass("vis").addClass("n-vis"),$applozic("#mck-tab-conversation").removeClass("n-vis").addClass("vis"),$applozic(".faq-common").removeClass("vis").addClass("n-vis"),
