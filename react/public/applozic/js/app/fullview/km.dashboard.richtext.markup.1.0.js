...
kommunicateDashboard.markup.getCarouselMarkup = function(options) {
    ...
    if (options && options.payload) {
        ...
        for (var i = 0; i < cards.length; i++) {
            cards[i].header && (headerOverlayTextClass = cards[i].header.overlayText ? (cards[i].header.imgSrc ? "km-dashboard-carousel-card-overlay-text ":"km-dashboard-carousel-card-overlay-text  km-dashboard-carousel-card-overlay-text-without-img") : "n-vis");     
            carouselHeaderClass = cards[i].header ? (cards[i].header.imgSrc ? "":"km-dashboard-carousel-card-header-without-img" ): "n-vis";  
            carouselInfoWrapperClass = cards[i].header ? "": "km-dashboard-carousel-card-info-wrapper-without-header";
            carouselInfoWrapperClass = cards[i].buttons ? carouselInfoWrapperClass.concat(" km-dashboard-carousel-card-info-wrapper-with-buttons"): "";
            cards[i].header && (headerImageClass = cards[i].header.imgSrc ? "km-dashboard-carousel-card-img": "n-vis");
            ...
            cards[i]["cardDescriptionClass"] = cards[i].description ? "km-dashboard-carousel-card-description-wrapper" : "n-vis";
            cardHtml["carouselHeaderClass"] = carouselHeaderClass;
            cardHtml["carouselInfoWrapperClass"] = carouselInfoWrapperClass;
            cards[i].header && (cardHtml.header = kommunicateDashboard.markup.cardHeader(cards[i].header));
            cardHtml.info = kommunicateDashboard.markup.cardInfo(cards[i]);
