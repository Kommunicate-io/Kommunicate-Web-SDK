...
(function ($kmApplozic, w, d) {
	...
	function Applozic(appOptions) {
		...
		var MCK_LOAD_INITIAL_CONVERSATION_STATE = kmUtils.getItemFromLocalStorage('KM_USER_SESSION').loadInitialStateConversation;
		var typeaheadMap = {};
		...
		_this.addGroupMember = function (params) {
			...
			if (typeof params.callback === 'function') {
				...
				if (params.groupId) {
					...
					conversationDetail.callback = params.callback;
				...
				kmUtils.ajax({
					...
					success: function (result) {
						...
						if (typeof result === 'object' && result !== null && result.token) {
							...
							if (result && result.imageLink && result.imageLink !== "") {
								...
								userSession && (userSession.imageLink = result.imageLink);
		...
		function MckMessageService() {
			...
			document.getElementById("km-clear-search-text").addEventListener('click',function(event){
				...
				document.getElementById("km-search-results").innerHTML = "";
				var activeConversationTab = document.getElementById(MESSAGE_SEARCH_DETAILS.activeConversationList);
			...
			_this.init = function () {
				...
				$kmApplozic(d).on("click", ".left .person,." + MCK_LAUNCHER + ",.km-conversation-tab-link, .km-contact-list ." + MCK_LAUNCHER, function (e) {
					...
					var $this = $kmApplozic(this);
					var tabId = $this.data("km-id");
					var tabViewId = $this.data("km-tab-viewid");
					tabId = (typeof tabId !== "undefined" && tabId !== "") ? tabId.toString() : "";
				...
				$kmApplozic(d).on('click', '.km-group-info-btn', function (e) {
					...
					$kmApplozic(".km-emoji-menu").css('width', "calc(100% - 10px)");
				});
			...
			_this.loadSupportGroup = function (params, callback) {
				...
				var isConvReq = false;
				var conversationList = typeof params.startTime == "number" ? "load-old-conversation" : "km-all-conversation-list"
				var reqData = '';
				...
				kmUtils.ajax({
					...
					success: function (data) {
						...
						if(data.errorResponse && data.errorResponse[0].errorCode == "AL-G-01" && typeof callback =="function") {
							...
							callback(data.errorResponse[0].errorCode);
						}
						...
						if (individual) {
							if (typeof currTabId === "undefined" || (params.tabId === currTabId && ('' + isGroupTab === '' + params.isGroup))) {
								...
								if (data + '' !== "null" && data.status !== 'error') {
									...
									if (data.groupFeeds.length > 0) {
										var loggedinUserId = MCK_USER_ID;
			...
			_this.emptyStateChange = function () {
				...
				if(typeof MCK_INTEGRATION_STARTED !== "undefined" && !MCK_INTEGRATION_STARTED) {
					...
					emptyStateDivFirstPara.innerHTML = "You have no pending conversations";
				}
			};
		}
		...
		function MckMessageLayout() {
			...
			_this.loadTab = function (params, callback) {
				...
				if (params.initialcall) {
					...
					mckMessageService.loadAssignedGroup(params, function() {
						mckMessageService.checkEmptyState();
					});
					mckMessageService.loadCloseGroup(params, function() {
						mckMessageService.checkEmptyState();
					});
				} else {
			...
			_this.addMessage = function (msg, contact, append, scroll, appendContextMenu) {
				...
				var msgList = [{
					...
					replyIdExpr: replyId,
					createdAtTimeExpr: kmDateUtils.getDate(msg.createdAtTime),
			...
			_this.initAutoSuggest = function (params) {
				...
				$searchId.mcktypeahead({
					...
					matcher: function (item) {
						...
						var contactNameArray = contact.displayName.toString().split(' ');
			...
			_this.updateContact = function (contact, message, $listId, update) {
				...
				if (message && message.createdAtTime > currentMessageTime || update) {
					...
					if (unreadCount > 0) {
						...
						$kmApplozic(".km-li-" + section + "-" + contHtmlExpr).addClass("km-unread-msg");
					}
			...
			_this.addContact = function (contact, $listId, message, prepend) {
				if (!MCK_LOAD_INITIAL_CONVERSATION_STATE && message && message.metadata && message.metadata.skipBot == "true" && contact && contact.metadata && contact.metadata.CONVERSATION_STATUS == KOMMUNICATE_CONSTANTS.CONVERSATION_STATE.INITIAL) {
					return;
				}
				var emoji_template = "";
				var conversationId = "";
				...
				var unreadCountClass = (unreadCount > 0 && $listId !== "km-contact-search-list") ? "km-unread-msg" : "";
				var olStatus = "n-vis";
				...
				var contactList = [{
					...
					unreadCountClassExpr: unreadCountClass,
				...
				if ($listId === "km-contact-search-list") {
					...
					if (typeof latestCreatedAtTime === "undefined" || (message ? message.createdAtTime : "") > latestCreatedAtTime || prepend) {
						$kmApplozic.kmtmpl("KMcontactTemplate", contactList).prependTo('#' + $listId);
						if (document.getElementById("conversation-section") && document.getElementById("conversation-section").getElementsByClassName("chat km-message-inner " + contact.contactId).length == 0) {
				...
				$kmApplozic("."+$listId).each(function(){
					$kmApplozic(this).html($kmApplozic(this).children('li').sort(function(a, b){
						return ($kmApplozic(b).data('msg-time')) > ($kmApplozic(a).data('msg-time')) ? 1 : -1;
					}));
				  });
			...
			_this.getMessageTextForContactPreview = function (message, contact, size) {
				...
				if (typeof message !== 'undefined') {
					if (message.message) {
						...
						if (message.contentType === 2) {
							...
							emoji_template = result ? (mailSvg + result[0]) : (mailSvg + "Email");
						}
				...
				return senderName + emoji_template;
			};
				...
				if (typeof message !== 'undefined') {
					if (message.contentType === 2) {
						...
						if (kmUtils.startsWith(msg, "<img")) {
							...
							emoji_template = result ? (result[0]) : ("Email");
						}
						else {
							var x = d.createElement('div');
							x.innerHTML = msg;
			...
			_this.populateMessage = function (messageType, message, notifyUser, list) {
				...
				$kmApplozic(".km-typing-indicator-in-preview").removeClass("vis").addClass("n-vis");
				$kmApplozic(".km-typing-indicator-for-agent--container").addClass("n-vis").removeClass("vis");
				...
				if (isBlocked) {
					...
					if (!MCK_BLOCKED_BY_MAP[tabId] && (w.MCK_OL_MAP[tabId] || MCK_LAST_SEEN_AT_MAP[tabId])) {
						if (w.MCK_OL_MAP[tabId]) {
							...
							var lastSeenAt = kmDateUtils.getLastSeenAtStatus(MCK_LAST_SEEN_AT_MAP[tabId]);
		...
		function MckInitializeChannel($this) {
			...
			var SOCKET = "";
			var subscriber = null;
			var supportChannelSubscriber =null;
			...
			_this.supportChannelSubscribedMessages = function (obj) {
				$mck_message_inner = mckMessageLayout.getMckMessageInner();
				var resp = $kmApplozic.parseJSON(obj.body);
				var message = resp.message;
				var messageType = resp.type;
				if (messageType === "APPLOZIC_01" || messageType === "APPLOZIC_02" || messageType === "MESSAGE_RECEIVED") {
					var messageArray = [];
			...
			_this.onMessage = function (obj) {
				...
				if (messageType === "APPLOZIC_04" || messageType === "MESSAGE_DELIVERED") {
					...
					$kmApplozic("#km-li-user-" + contact.htmlId).addClass("km-unread-msg");
						...
						if (status === BLOCK_STATUS_MAP[2]) {
							...
							if (!MCK_BLOCKED_TO_MAP[tabId]) {
								if (w.MCK_OL_MAP[tabId]) {
									...
									$mck_tab_status.html(kmDateUtils.getLastSeenAtStatus(MCK_LAST_SEEN_AT_MAP[tabId]));
								}
								$mck_tab_title.addClass('km-tab-title-w-status');
								$mck_tab_status.removeClass('n-vis').addClass('vis');
							}
					...
					if (messageType === "APPLOZIC_03") {
						...
						var contact = (message.groupId) ? kmGroupUtils.getGroup(message.groupId) : mckMessageLayout.getContact(message.to);
						if (contact && message.metadata && message.metadata.KM_ASSIGN) {
						...
						if (contact && message.metadata && message.metadata.KM_STATUS) {
							contact.metadata.CONVERSATION_STATUS = KOMMUNICATE_CONSTANTS.CONVERSATION_STATUS[message.metadata.KM_STATUS]+"";
							contact && (KM_GROUP_MAP[contact.contactId]=contact);
						}
						var $mck_sidebox_content = $kmApplozic("#km-sidebox-content");
						var tabId = $mck_message_inner.data('km-id');
