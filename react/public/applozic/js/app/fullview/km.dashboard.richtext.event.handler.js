 ...
 */
const km_actionable_message_dashboard_svgs = {
    arrow:{
...
kommunicateDashboard.richMsgEventHandler ={
    initializeSlick:function($cardMessageContainer){
        if ($cardMessageContainer.length >= 0) {
            var slider = tns({
                ...
                slideBy: 1,
                loop:false,
                controlsText:[km_actionable_message_dashboard_svgs.arrow.left, km_actionable_message_dashboard_svgs.arrow.left],
                "mouseDrag": true,
                "arrowKeys": true,
                onInit: function () {
    ...
    decrementGuestCount: function(e) {
        ...
        target.parentElement.getElementsByClassName('km-dashboard-room-number-field')[0].stepDown();
    },
    incrementGuestCount: function (e) {
        var  target = e.target || e.srcElement;
        target.parentElement.getElementsByClassName('km-dashboard-room-number-field')[0].stepUp();
    },
    ...
    addMoreRoom : function(e){
       ...
       roomInfoElem.innerHTML=kommunicateDashboard.markup.getSingleRoomPaxInfo(roomCount);
       container.getElementsByClassName('km-dashboard-room-person-selector-container')[0].appendChild(roomInfoElem);
    },
    processSelectedRoom:function(e){
        ...
        var roomGuestJson =[];
       //var roomGuest= document.querySelectorAll(".km-room-person-selector-container input.km-dashboard-room-number-field");
       var roomGuest = $(e.target).closest('.km-dashboard-msg-box-rich-text-container').find('.km-dashboard-room-person-selector-container input.km-dashboard-room-number-field');
       var NoOfChild = $(e.target).closest('.km-dashboard-msg-box-rich-text-container').find('.km-dashboard-room-person-selector-container input.km-dashboard-person-number-field');
       // TODO: process number of child if required
        ...
        for (var i = 0; i < roomGuest.length; i++) {
            ...
            message += "Room " + (i + 1) + " Guest " + roomGuest[i].value + " Children " + noOfChild + "\n";
       }
