...
class Integration extends Component {
  ...
  sendMail = (e) => {
    ...
    if(this.state.multipleEmailAddress.length >= 1){
      ...
      notifyThatEmailIsSent({to:this.state.multipleEmailAddress,templateName:"SEND_KOMMUNICATE_SCRIPT"}).then((data=>{
      ...
      if(!isEmail(this.state.emailAddress)){
        ...
        notifyThatEmailIsSent({to:this.state.emailAddress,templateName:"SEND_KOMMUNICATE_SCRIPT"}).then((data=>{
          _this.setState({multipleEmailAddress:[], emailAddress:""});
        }));
      }
    }
               ...
               <div>
                <form>
                  ...
                  </div>
                  {
                    ...
                    <div className="form-group">
                      <button type ="button" className="btn btn-primary" onClick={this.copyToClipboard}>{this.state.copySuccess}</button>
                    </div>
                  }
                  <div className="form-group">
                  ...
                  <div className={((this.state.emailInstructions === true)  ? 'form-group row' : 'n-vis')} style={{"marginLeft": "0"}}>
                    ...
                    */}
                    <div className="form-group col-md-5 multiple-email-container">
                      ...
                      <input className="input-email" value={this.state.emailAddress} onKeyDown={this.checkForSpace} onChange={this.multipleEmailHandler}  placeholder="Enter email here"/>
                    </div>
                    <div className="col-md-7">
                      <button className="btn btn-sm btn-primary px-4 m-t-1px" onClick={this.sendMail}> Send </button>
                    </div>
                  </div>
                </form>
              </div>
