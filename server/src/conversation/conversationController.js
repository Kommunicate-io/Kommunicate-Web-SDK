...
exports.getConversationStats = (req, res) => {
    ...
    return conversationService.getConversationStats(agentId, customerId, applicationId, startTime, endTime).then(response => {
...
exports.getConversationStat = (req, res) => {
    if (!req.query.days) {
        let days = req.query.daily == "true" ? (new Date()).getDate() : (new Date()).getDay();
        req.query.days = days;
    }
