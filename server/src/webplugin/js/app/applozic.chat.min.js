...
ALApiService.createUserFriendList=function(options){ALApiService.ajax({url:MCK_BASE_URL+FRIEND_LIST_URL+options.data.group.groupName+"/add/",type:"POST",async:typeof options.async!=="undefined"?options.async:true,global:false,data:JSON.stringify(options.data.group.groupMemberList),contentType:"application/json",success:function(response){if(options.success){options.success(response);ALStorage.setFriendListGroupName(options.data.group.groupName)}},error:function(response){if(options.error)options.error(response)}})};
ALApiService.createOpenFriendList=function(options){ALApiService.ajax({url:MCK_BASE_URL+FRIEND_LIST_URL+options.data.group.groupName+"/add/members",type:"POST",data:JSON.stringify(options.data.group),async:typeof options.async!=="undefined"?options.async:true,global:false,contentType:"application/json",success:function(response){if(options.success){options.success(response);ALStorage.setFriendListGroupName(options.data.group.groupName);ALStorage.setFriendListGroupType(options.data.group.type)}},error:function(response){if(options.error)options.error(response)}})};
ALApiService.getFriendList=function(options){var getFriendListUrl=options.data.type!=="null"?"/get?groupType\x3d9":"/get";options.data.url=options.data.url?options.data.url:getFriendListUrl;ALApiService.ajax({url:MCK_BASE_URL+FRIEND_LIST_URL+options.data.groupName+options.data.url,type:"GET",async:typeof options.data.async!=="undefined"?options.data.async:true,global:false,contentType:"application/json",success:function(response){if(options.success)options.success(response)},error:function(response){if(options.error)options.error(response)}})};
ALApiService.removeUserFromFriendList=function(options){var getFriendListUrl=options.group.type?"/remove?userId\x3d"+options.group.userId+"\x26groupType\x3d9":"/remove?userId\x3d"+options.group.userId;ALApiService.ajax({url:MCK_BASE_URL+FRIEND_LIST_URL+options.group.groupName+getFriendListUrl,type:"Post",contentType:"application/json",async:typeof options.async!=="undefined"?options.async:true,global:false,success:function(response){if(options.success)options.success(response)},error:function(response){if(options.error)options.error(response)}})};
...
(function(window){function define_ALSocket(){var ALSocket={};var MCK_APP_ID;ALSocket.events={};var subscriber=null;ALSocket.stompClient=null;var TYPING_TAB_ID="";ALSocket.typingSubscriber=null;var openGroupSubscriber=[];var checkConnectedIntervalId;var sendConnectedStatusIntervalId;var OPEN_GROUP_SUBSCRIBER_MAP;ALSocket.mck_typing_status=0;var MCK_TYPING_STATUS;var SOCKET="";var MCK_WEBSOCKET_URL="https://apps.applozic.com";var MCK_TOKEN;var USER_DEVICE_KEY;var mckUtils=new MckUtils;ALSocket.init=
function(appId,data,_events){if(appId)MCK_APP_ID=appId;if(typeof data!=="undefined"){MCK_TOKEN=data.token;USER_DEVICE_KEY=data.deviceKey;MCK_WEBSOCKET_URL=data.websocketUrl}ALSocket.events=_events;if(typeof MCK_WEBSOCKET_URL!=="undefined"){var port=!mckUtils.startsWith(MCK_WEBSOCKET_URL,"https")?"15674":"15675";if(typeof SockJS==="function"){if(!SOCKET)SOCKET=new SockJS(MCK_WEBSOCKET_URL+":"+port+"/stomp");ALSocket.stompClient=Stomp.over(SOCKET);ALSocket.stompClient.heartbeat.outgoing=0;ALSocket.stompClient.heartbeat.incoming=
...
checkConnectedIntervalId=setInterval(function(){ALSocket.connectToSocket(isFetchMessages)},6E5);sendConnectedStatusIntervalId=setInterval(function(){ALSocket.sendStatus(1)},12E5)}else ALSocket.connectToSocket(isFetchMessages)};ALSocket.connectToSocket=function(isFetchMessages){if(typeof ALSocket.connectToSocket==="function")ALSocket.events.connectToSocket(isFetchMessages)};ALSocket.stopConnectedCheck=function(){if(checkConnectedIntervalId)clearInterval(checkConnectedIntervalId);if(sendConnectedStatusIntervalId)clearInterval(sendConnectedStatusIntervalId);
...
MCK_APP_ID+","+MCK_USER_ID+","+status);ALSocket.mck_typing_status=status}};ALSocket.onTypingStatus=function(resp){if(typeof ALSocket.events.onTypingStatus==="function")ALSocket.events.onTypingStatus(resp)};ALSocket.reconnect=function(){ALSocket.unsubscibeToTypingChannel();ALSocket.unsubscibeToNotification();ALSocket.disconnect();var data={};data.token=MCK_TOKEN;data.deviceKey=USER_DEVICE_KEY;data.websocketUrl=MCK_WEBSOCKET_URL;ALSocket.init(MCK_APP_ID,data,ALSocket.events)};ALSocket.onError=function(err){console.log("Error in channel notification. "+
err);if(typeof ALSocket.events.onConnectFailed==="function")ALSocket.events.onConnectFailed()};ALSocket.sendStatus=function(status){if(ALSocket.stompClient&&ALSocket.stompClient.connected)ALSocket.stompClient.send("/topic/status-v2",{"content-type":"text/plain"},MCK_TOKEN+","+USER_DEVICE_KEY+","+status)};ALSocket.onConnect=function(){if(ALSocket.stompClient.connected){if(subscriber)ALSocket.unsubscibeToNotification();subscriber=ALSocket.stompClient.subscribe("/topic/"+MCK_TOKEN,ALSocket.onMessage);
ALSocket.sendStatus(1);ALSocket.checkConnected(true)}else setTimeout(function(){subscriber=ALSocket.stompClient.subscribe("/topic/"+MCK_TOKEN,ALSocket.onMessage);ALSocket.sendStatus(1);ALSocket.checkConnected(true)},5E3);if(typeof ALSocket.events.onConnect==="function")ALSocket.events.onConnect()};ALSocket.onOpenGroupMessage=function(obj){if(typeof ALSocket.events.onOpenGroupMessage==="function")ALSocket.events.onOpenGroupMessage(obj)};ALSocket.onMessage=function(obj){if(subscriber!=null&&subscriber.id===
obj.headers.subscription){var resp=JSON.parse(obj.body);var messageType=resp.type;if(typeof ALSocket.events.onMessage==="function")ALSocket.events.onMessage(resp);if(messageType==="APPLOZIC_04"||messageType==="MESSAGE_DELIVERED")ALSocket.events.onMessageDelivered(resp);else if(messageType==="APPLOZIC_08"||messageType==="MT_MESSAGE_DELIVERED_READ")ALSocket.events.onMessageRead(resp);else if(messageType==="APPLOZIC_05")ALSocket.events.onMessageDeleted(resp);else if(messageType==="APPLOZIC_27")ALSocket.events.onConversationDeleted(resp);
else if(messageType==="APPLOZIC_11")ALSocket.events.onUserConnect(resp.message);else if(messageType==="APPLOZIC_12"){var lastSeenAtTime=resp.message.split(",")[1];ALSocket.events.onUserDisconnect({"userId":userId,"lastSeenAtTime":lastSeenAtTime})}else if(messageType==="APPLOZIC_29")ALSocket.events.onConversationReadFromOtherSource(resp);else if(messageType==="APPLOZIC_28")ALSocket.events.onConversationRead(resp);else if(messageType==="APPLOZIC_16"){var status=resp.message.split(":")[0];var userId=
resp.message.split(":")[1];ALSocket.events.onUserBlocked({"status":status,"userId":userId})}else if(messageType==="APPLOZIC_17"){var status=resp.message.split(":")[0];var userId=resp.message.split(":")[1];ALSocket.events.onUserUnblocked({"status":status,"userId":userId})}else if(messageType==="APPLOZIC_18")ALSocket.events.onUserActivated();else if(messageType==="APPLOZIC_19")ALSocket.events.onUserDeactivated();else{var message=resp.message;if(messageType==="APPLOZIC_03")ALSocket.events.onMessageSentUpdate({"messageKey":message.key});
else if(messageType==="APPLOZIC_01"||messageType==="MESSAGE_RECEIVED"){var messageFeed=alMessageService.getMessageFeed(message);ALSocket.events.onMessageReceived({"message":messageFeed})}else if(messageType==="APPLOZIC_02"){var messageFeed=alMessageService.getMessageFeed(message);ALSocket.events.onMessageSent({"message":messageFeed})}}}};return ALSocket}if(typeof ALSocket==="undefined")window.Applozic.ALSocket=define_ALSocket();else console.log("ALSocket already defined.")})(window);
var mckNotificationUtils=new MckNotificationUtils;var alNotificationService=new AlNotificationService;
...
_this.removeGroupMemberFromChat=function(params){var group={};var response=new Object;if(params.groupId)group.groupId=params.groupId;else if(params.clientGroupId)group.clientGroupId=params.clientGroupId;else{response.status="error";response.errorMessage="GroupId or Client GroupId Required";if(typeof params.callback==="function")params.callback(response);return}group.userId=params.userId;Applozic.ALApiService.removeGroupMember({data:group,success:function(response){if(response.status==="success"){if(params.clientGroupId){var group=
...
success:function(){},error:function(){}})};_this.fetchConversationByTopicId=function(data,callback){window.Applozic.ALApiService.fetchConversationByTopicId({data:data,success:function(data){if(typeof data==="object"&&data.status==="success"){var conversationList=data.response;if(conversationList.length>0)$applozic.each(conversationList,function(i,conversationPxy){MCK_CONVERSATION_MAP[conversationPxy.id]=conversationPxy;MCK_TOPIC_CONVERSATION_MAP[conversationPxy.topicId]=[conversationPxy.id];if(conversationPxy.topicDetail)try{MCK_TOPIC_DETAIL_MAP[conversationPxy.topicId]=
