...
exports.goAway = (userId, appId)=>{
    return Promise.resolve(getCustomerInfoByApplicationId(appId)).then(customer=>{
      if(!customer) {
        ...
        return Promise.resolve(userModel.update({ availabilty_status: 0 }, {where: {"userName": userId, customerId: customer.id}})).then(result=>{
...
exports.goOnline = (userId, appId)=>{
    return Promise.resolve(getCustomerInfoByApplicationId(appId)).then(customer=>{
      if(!customer) {
        ...
        return Promise.resolve(userModel.update({ availabilty_status: 1 }, {where: {"userName": userId, customerId: customer.id}})).then(result=>{
