...
let joinKommunicateUrl = config.getProperties().urls.dashboardHostUrl+"/signup?invite=true&applicationId=:applicationId&token=:token?referer=:referer"
    ...
    try{
        ...
        if(!html){
            switch(options.templateName){
                ...
                templateReplacement[":kmWebsiteLogoUrl"] = kmWebsiteLogoUrl,
                templateReplacement[":joinKommunicateUrl"] =joinKommunicateUrl.replace(":applicationId",options.applicationId).replace(":token",custInfo.apzToken).replace(":referer",options.agentId),
                options.templatePath = templatePath,
                options.templateReplacement = templateReplacement;
