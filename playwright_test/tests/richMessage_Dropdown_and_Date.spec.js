...
// Testing action Form Template
  test("Actions in Form Template", async ({ request }) => {
    const iframe = page.frameLocator(WIDGET_LOCATORS.kmIframe)
    await iframe.locator(RICHMESSAGES_LOCATORS.kmDropDownInFormTemplate).selectOption({ label: 'Entrepreneur' })
                // .click();
    await iframe.locator(RICHMESSAGES_LOCATORS.kmTextAreaInFormTemplate)
                .click();
    await iframe.locator(RICHMESSAGES_LOCATORS.kmTextAreaInFormTemplate)
