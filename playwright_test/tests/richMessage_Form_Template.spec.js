...
import { WIDGET_LOCATORS, LOCATORS, COMMON_VALUES, RICHMESSAGES_LOCATORS } from '../locaterPackage/kmLocators.js';
...
// Testing action Form Template
  test("Actions in Form Template", async ({ request }) => {
    const iframe = page.frameLocator(WIDGET_LOCATORS.kmIframe)
    await iframe.locator(RICHMESSAGES_LOCATORS.kmNameInFormTemplate)
                .click();
    await iframe.locator(RICHMESSAGES_LOCATORS.kmNameInFormTemplate)
                .type("Test User");
    await iframe.locator(RICHMESSAGES_LOCATORS.kmPhoneInFormTemplate)
                .click();
    await iframe.locator(RICHMESSAGES_LOCATORS.kmPhoneInFormTemplate)
                .type("1234567890");
    await iframe.locator(RICHMESSAGES_LOCATORS.kmRadioButtonInFormTemplate)
                .click();
    await iframe.locator(RICHMESSAGES_LOCATORS.kmCheckBox1InFormTemplate)
                .click();
    await iframe.locator(RICHMESSAGES_LOCATORS.kmCheckBox2InFormTemplate)
                .click();
    await iframe.locator(RICHMESSAGES_LOCATORS.kmSubmitBtnInFormTemplate)
                .click();
