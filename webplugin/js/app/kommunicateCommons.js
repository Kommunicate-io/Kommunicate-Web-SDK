...
function KommunicateCommons() {
    ...
    function stringStartsWith(value, prefix) {
        return (
            typeof value === 'string' && typeof prefix === 'string' && value.indexOf(prefix) === 0
        );
    }
    ...
    function getPreviewWhitelistHosts() {
        ...
        function addEntries(value) {
            ...
            whitelist.push(normalizeEntryHostname(value));
        }
        addEntries(globals.previewWhitelist);
        addEntries(globals.widgetSettings?.previewWhitelist);
        (globals.appSettings?.chatWidget?.previewWhitelist || []).forEach(addEntries);
        return whitelist.filter(function (hostname) {
            return !!hostname;
        });
    }
    ...
    function isHostnameWhitelisted(hostname) {
        ...
        for (var idx = 0; idx < whitelist.length; idx++) {
            ...
            if (
                allowedHost.length < normalizedHostname.length &&
                stringEndsWith(normalizedHostname, '.' + allowedHost)
            ) {
                return true;
            }
        }
    ...
    function isHostnameBlocked(hostname) {
        ...
        if (isIpv4Blocked(normalized) || isIpv6Blocked(normalized)) {
            return true;
        }
        return false;
    }
