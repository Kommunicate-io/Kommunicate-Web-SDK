    ...
    _this.zopimEvents = function (eventDetails) {
        ...
        if (
            (CURRENT_GROUP_DATA.createdAt &&
                eventDetails.timestamp < CURRENT_GROUP_DATA.createdAt) ||
            eventDetails.nick == 'visitor'
        ) {
            return;
        }
        _this.updateNumberInZopim();
        if (eventDetails.type == 'chat.msg') {
            //If agent sends normal message
            _this.handleZendeskAgentMessageEvent(eventDetails);
        } else if (eventDetails.type == 'chat.file') {
            //If agent sends file attachments
            _this.handleZendeskAgentFileSendEvent(eventDetails);
        } else if (eventDetails.type == 'chat.memberleave') {
            //If agent leaves conversation
...
var onTabClickedHandlerForZendeskConversations = function (event) {
    ...
    if (kommunicate._globals.zendeskChatSdkKey) {
        ...
        var assigneeInfo =
            currentGroupData &&
            currentGroupData.users &&
            Object.values(currentGroupData.users).find(function (member) {
