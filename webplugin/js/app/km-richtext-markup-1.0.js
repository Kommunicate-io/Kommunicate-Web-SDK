   ...
   {{/payload}}
    ...
    getFormTemplate: function () {
        return `<div class="mck-msg-box-rich-text-container mck-form-template-container" data-hidePostFormSubmit="{{hidePostFormSubmit}}">
                <form class="km-btn-hidden-form mck-actionable-form {{#isHideFormContainer}}n-vis{{/isHideFormContainer}}" action="{{actionUrl}}" method="post" data-msg-key={{msgKey}} >
    ...
    if (metadata && metadata.payload) {
        ...
        if (json.elements && json.elements.length) {
            ...
            json.elements = json.elements.map(function (item) {
                ...
                item.description &&
                    (item.description = kommunicateCommons.removeHtmlTag(
                        item.description
                    ));
